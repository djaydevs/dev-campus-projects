...
function Add() {
    ...
    if(detailsEntered == ""){
        alert("Pleaser complete your details");
    } else {
        if(row == null){
            insert(readDetails);
            alert("Record Added");
...
function retrieveDetails() {
 ...
 //READ DETAILS FROM ARRAY TO LOCAL STORAGE
 ...
 function insert(readDetails) {
     ...
     row.insertCell(8).innerHTML = `<button class="edit-btn" onclick = edit(this)>Edit</button>
     <button class="delete-btn" onclick = remove(this)>Delete</button>`
...
function update(th) {
    ...
    row.cells[2].innerHTML = document.getElementById("fName").value;
